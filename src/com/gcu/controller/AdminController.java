...
public class AdminController {
	...
	@RequestMapping(path="/salesMenu", method=RequestMethod.GET)
	public ModelAndView NavToSalesMenu(ModelMap model) {
		if(session.getAttribute("admin") == null || !(boolean)session.getAttribute("admin"))
			return NavToHomePage();
		
		model.addAttribute("sales", pbs.GetSales());
		model.addAttribute("newSale", new Sale());
		return new ModelAndView("salesMenu", "model", model);
	}
	...
	private ModelAndView NavToHomePage() {
